...
struct Node {
    ...
    void setChild(int direction, Node* child){
        // cout << "Setting child " << child->key<< " " << child->value<<endl;
        if(direction > 0){
            right = child;
        } else if(direction < 0){
            left = child;
...
class AVLTree {
    ...
    private:
        ...
        //Search
        Result attemptGet(key_type key, Node* node, int dir, long nodeV, value_type* &val_p);
...
template<typename T, int Threads>
void AVLTree<T, Threads>::releaseAll(){
...
Node* AVLTree<T, Threads>::newNode(key_type key){
    // cout << "New node with key " << key << endl;
...
Node* AVLTree<T, Threads>::newNode(int height, key_type key, long version, value_type* value, Node* parent, Node* left, Node* right){
    // cout << "New node with " << key << ":" << value << endl;
...
//                 Result vo = attemptGet(key, right, rightCmp, ovl, nullptr);
//                 if(vo != RETRY){
//                     return vo == FOUND;
//                 }
//             }
...
template<typename T, int Threads>
Result AVLTree<T, Threads>::attemptGet(key_type key, Node* node, int dir, long nodeV, value_type* &val_p){
    while(true){
        ...
        if(!child){
            ...
            if(isShrinkingOrUnlinked(childOVL)){
                ...
                Result result = attemptGet(key, child, childCmp, childOVL, val_p);
                if(result != RETRY){
                    return result;
                }
            }
...
inline bool shouldUpdate(Function func, bool prev, value_type* /* expected*/){
    return func == UpdateIfAbsent ? !prev : prev;
}

inline Result updateResult(Function func, value_type* /* prev*/){
    return func == UpdateIfAbsent ? NOT_FOUND : FOUND;
}

inline Result noUpdateResult(Function func, value_type* /* prev*/){
    return func == UpdateIfAbsent ? FOUND : NOT_FOUND;
}

template<typename T, int Threads>
bool AVLTree<T, Threads>::add(key_type key, value_type* value){
    // cout << "Adding " << key << ":" << value <<endl;
    return updateUnderRoot(key, UpdateIfAbsent, value, rootHolder) == NOT_FOUND;
}

template<typename T, int Threads>
bool AVLTree<T, Threads>::remove(key_type key){
    return updateUnderRoot(key, UpdateIfPresent, nullptr, rootHolder) == FOUND;
}
...
Result AVLTree<T, Threads>::attemptUpdate(key_type key, Function func, value_type* newValue, Node* parent, Node* node, long nodeOVL){
    ...
    while(true){
        ...
        if(!child){
            // cout << "Searching key " << key <<" Found child " << child->key <<endl;
            if(newValue == nullptr){
                return NOT_FOUND;
            } else {
                bool success;
...

                {
                    ...
                    if(node->child(cmp)){
                        ...
                        Node* newChild = newNode(1, key, 0, newValue, node, nullptr, nullptr);
            ...
            if(isShrinkingOrUnlinked(childOVL)){
                ...
                Result vo = attemptUpdate(key, func, newValue, node, child, childOVL);
                if(vo != RETRY){
                    return vo;
                }
            }
...
Result AVLTree<T, Threads>::attemptNodeUpdate(Function func, value_type* newValue, Node* parent, Node* node){
    ...
    if(!newValue && (!node->left || !node->right)){
        ...
        value_type* prev = node->value;
        // if(!shouldUpdate(func, prev, expected)){
        //     releaseAll();
        //     return noUpdateResult(func, prev);
        // }

        if(newValue == nullptr && (!node->left || !node->right)){
            releaseAll();
            return RETRY;
        }
