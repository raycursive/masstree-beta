...
using lcdf::Str;
using mytree = fourtree::four_tree<fourtree::tree_params<>>;

inline void put(mytree* bt, threadinfo* ti, fourtree::query& q, long k, long v) {
...
int main() {
  ...
  // test fixed sized key
  // long arr[5];
  // fix_sized_key<16, true> keys[5];
  // fix_sized_key<16, false> keys2[5];
  // for (int i = 0; i < 5; i++) {
  //   arr[i] = rand();
  //   keys[i] = fix_sized_key<16, true>(quick_istr(arr[i]).string());
  //   keys2[i] = fix_sized_key<16, false>(quick_istr(arr[i]).string());
  // }

  // for (int i = 0; i < 5; i++) {
  //   for (int j=0; j<5; j++) {
  //     std::cout << "raw compare " << arr[i] << " and " << arr[j] << ": " << (arr[i] == arr[j] ? 0 : arr[i] > arr[j] ? 1 : -1) << std::endl;
  //     std::cout << "kc1 compare " << arr[i] << " and " << arr[j] << ": " << keys[i].compare(keys[j]) << std::endl;
  //     std::cout << "kc2 compare " << arr[i] << " and " << arr[j] << ": " << keys2[i].compare(keys2[j]) << std::endl;
  //     std::cout << std::endl;
  //   }
  // }
  ...
  fourtree::query q;
  // put(bt, main_ti, q, 1, 2);
  // put(bt, main_ti, q, 3, 3);
  // put(bt, main_ti, q, 4, 4);
  // put(bt, main_ti, q, 6, 5);
  // bt->print(stdout);
